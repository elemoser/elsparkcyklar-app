...
describe("Api test suite", () => {
    ...
    it("DELETE /v1/invoice/id/[user_id] - Remove invoice", async () => {
        try {
            ...
            expect(
                removeNoneExisting,
                `Should fail as there is no invoice with id = ${NEVER_EXISTING_NUMBER}`
            ).to.have.status(404);
            const removed = await chai.request(app).delete(`${baseRoute}/id/5`);
            expect(
                removed,
                "Should succeed. Invoice id 5 should exist"
            ).to.have.status(200);
        } catch (error) {
            // Handle errors
            console.error(error);
    ...
    it("GET /invoice - GET all invoices", async () => {
        try {
             ...
             * Expect to succeed
             */
            const getInvoice = await chai.request(app).get(`${baseRoute}`);
    ...
    it("GET /invoice/:invoice_id - GET a specifik invoice with invoice ID", async () => {
        try {
            ...
            expect(getInvoice).to.have.status(
                200,
                "should succeed as the invoice exists"
            );
            expect(getInvoice.body.invoice).to.not.be.empty;
            ...
            expect(getNoneExistingInvoice).to.have.status(
                404,
                "should succeed as the Invoice exists"
            );
        } catch (error) {
    ...
    it("PUT /v1/invoice/id/:invoice_id - Update a invoice", async () => {
        ...
        try {
             ...
             * Expect to fail
             */
            const updateNonExistingInvoice = await chai
                .request(app)
             ...
             */
            const updateNothing = await chai
                .request(app)
                .put(`${baseRoute}/id/${ALWAYS_EXISTING_NUMBER}`)
                .send();
            expect(
                updateNothing,
                "Should be 200 but nothing is updated"
            ).to.have.status(200);

            /**
             * Make illegal status update
             */
            const updateWithBrokenData = await chai
                .request(app)
                .put(`${baseRoute}/id/${ALWAYS_EXISTING_NUMBER}`)
                .send(invoiceUpdateBrokenDataStatusOnly);
            expect(
                updateWithBrokenData,
                "Expecting 401 for update with broken status data"
            ).to.have.status(400);

            /**
             * Make illegal total_price update
             */
            const updateWithBrokenDataPrice = await chai
                .request(app)
                .put(`${baseRoute}/id/${ALWAYS_EXISTING_NUMBER}`)
                .send(invoiceUpdateBrokenDataPriceOnly);
            expect(
                updateWithBrokenDataPrice,
                "Expecting 401 for update with broken Price data"
            ).to.have.status(400);
        } catch (error) {
            console.error("Error in test:", error);
            throw error; // Re-throw the error to fail the test
