...
describe("Api test suite", () => {
    ...
    it("PUT /v1/users/id/:user_id - Update a users data", async () => {
        try {
             ...
             * Expect to fail
             */
            const updateNonExistingUser = await chai
                .request(app)
             ...
             * Expect to succeed
             */
            const updateExistingUser = await chai
                .request(app)
    ...
    it("GET /history/:user_id - GET users history from booking", async () => {
        try {
             ...
             */
            const getNoneExistingHistory = await chai
                .request(app)
                .get(`/v1/users/history/${NEVER_EXISTING_SSN}`);

            expect(getNoneExistingHistory).to.have.status(
                404,
                "Expect to fail as the user doesnt exist"
            );
            expect(getNoneExistingHistory.body.error).to.equal(
                "No matching id"
            );
             ...
             * Expect to succeed
             */
            const getHistory = await chai
                .request(app)
                .get(`/v1/users/history/${ALWAYS_EXISTING_SSN}`);
            expect(getHistory).to.have.status(
                200,
    ...
    it("GET /invoice/:user_id - GET users invoices", async () => {
        try {
             ...
             * Expect to fail as the user doesnt exist
             */
            const getNoneExistingInvoice = await chai
                .request(app)
                .get(`/v1/users/invoice/${NEVER_EXISTING_SSN}`);
            expect(
                getNoneExistingInvoice,
                `Expect to fail as the user: ${NEVER_EXISTING_SSN} doesnt exist`
            ).to.have.status(404); // Ger alltid minst [] även om användaren inte finns?
            expect(getNoneExistingInvoice.body.error).to.equal(
                "No matching id"
            );
             ...
             * Expect to succeed
             */
            const getInvoice = await chai
                .request(app)
                .get(`/v1/users/invoice/${ALWAYS_EXISTING_SSN}`);
            expect(getInvoice).to.have.status(
                200,
                "should succeed as the user exists"
            );
    ...
    it("GET /v1/users/name:name - Test to find a user named John (in first AND last name", (done) => {
        ...
        chai.request(app)
            ...
            .end((err, res) => {
                ...
                let findJohnFName = res.body.users.find((user) =>
                    user.first_name.toLowerCase().includes(nameCompare)
                );
                let findJohnLName = res.body.users.find((user) =>
                    user.last_name.toLowerCase().includes(nameCompare)
                );
