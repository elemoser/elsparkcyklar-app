...
describe("Api test suite for the city/ routes", () => {
    ...
    it("DELETE /v1/users/id/[user_id] - Remove invoice", async () => {
        try {
            ...
            expect(
                ...
                `Should fail as there is no city with id = ${NEVER_EXISTING_NUMBER}`
            ).to.be.equal("City doesn't exist");
            ...
            expect(
                removed,
                `Should succeed. City id ${testIdForUpdate} should exist`
            ).to.have.status(200);
            expect(
                removed.body.message,
                `Should succeed as city with id: ${testIdForUpdate} is created before each`
            ).to.equal("City successfully deleted");
        } catch (error) {
            // Handle errors
            console.error(error);
            throw error;
    ...
    it("GET /cities - GET all invoices", async () => {
        try {
             ...
             * Expect to succeed
             */
            const getCities = await chai.request(app).get(`${baseRoute}`);
            expect(getCities).to.have.status(
                200,
                "should succeed as the user exists"
            );
    ...
    it("GET /city/id/:city_id - GET a specific city with city_id", async () => {
        try {
             ...
             * Expect to succeed
             */
            const getCity = await chai
                .request(app)
                .get(`${baseRoute}/id/${ALWAYS_EXISTING_NUMBER}`);
            expect(
                getCity,
                `should succeed as the city 'Stockholm' ID: ${ALWAYS_EXISTING_NUMBER}  exists`
            ).to.have.status(200);
            ...
            const getNoneExistingCity = await chai
                .request(app)
                .get(`${baseRoute}/id/${NEVER_EXISTING_NUMBER}`);
            expect(
                getNoneExistingCity,
                `should Fail as the city with id ${NEVER_EXISTING_NUMBER} doesnt exists`
            ).to.have.status(404);
        } catch (error) {
            console.error("Error in test:", error);
            throw error; // Re-throw the error to fail the test
    ...
    it("POST /v1/city - Creating cities", async () => {
        ...
        const resFail = await chai.request(app).post(`${baseRoute}`).send();
        expect(
            resFail,
            "Create City with missing params. should fail"
        ).to.have.status(400);
        expect(resFail.body.error).to.equal("Missing required fields");

        const resFail2 = await chai
            .request(app)
            .post(`${baseRoute}`)
            .send({ id: `${NEVER_EXISTING_NUMBER}` });
        expect(
            resFail2,
            "Create City with only id params. should fail"
        ).to.have.status(400);
        expect(resFail2.body.error).to.equal("Missing required fields");

        const resFail3 = await chai
            .request(app)
            .post(`${baseRoute}`)
            .send(newCityBrokenBounds);
        expect(
            resFail3,
            `Create City,sending an array (not as string) as bounds`
        ).to.have.status(500);
        expect(resFail3.error).to.not.be.empty;

        const resSucceed = await chai
            .request(app)
            .post(`${baseRoute}`)
            .send(newCity);
        expect(
            resSucceed,
            `Create City with correct values ID: ${mockCityId}`
        ).to.have.status(200);
    ...
    it("PUT /v1/city/id/:city_id - Update a city", async () => {
        ...
        try {
             ...
             */
            const updateNonExistingCity = await chai
                .request(app)
                .put(`${baseRoute}/id/${NEVER_EXISTING_NUMBER}`)
                .send();
            expect(updateNonExistingCity, "It shouldnt exist").to.have.status(
                404
            );
            ...
            const updateExistingCityNoVal = await chai
                .request(app)
                .put(`${baseRoute}/id/1`)
                .send();
            expect(
                updateExistingCityNoVal,
                "It shouldnt change anything since its empty, but it should succeed"
            ).to.have.status(200);
            const getCity1AfterEmptyUpdate = await chai
                .request(app)
                .get(`${baseRoute}/id/1`);
            expect(
                getCity1.body.city.name,
                "Should be equal, the update was empty so city 1 should still be named 'Stockholm'"
             ...
             */
            const updateExistingCityBrokenVal = await chai
                .request(app)
                .put(`${baseRoute}/id/1`)
                .send(updateCityBoundsNotString);
            expect(
                updateExistingCityBrokenVal,
                "Should fail bc the provided bounds are not strings"
            ).to.have.status(500);

            /**
             * Expect to succeed
             */
            const updateExistingCity = await chai
                .request(app)
                .put(`${baseRoute}/id/${testIdForUpdate}`)
                .send(updateCity);
            expect(updateExistingCity).to.have.status(200);
            expect(updateExistingCity.body.message).to.equal(
